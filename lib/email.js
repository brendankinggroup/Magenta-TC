...
const DETAIL_SECTIONS = {
  'new-file': [
    ...
    { title: 'Buyer(s)', fields: [
      ...
      ['buyer3Name', 'Buyer 3 Name'],
      ['buyer3Email', 'Buyer 3 Email'],
      ['buyer3Phone', 'Buyer 3 Phone'],
      ['buyer4Name', 'Buyer 4 Name'],
      ['buyer4Email', 'Buyer 4 Email'],
      ['buyer4Phone', 'Buyer 4 Phone'],
    ...
    { title: 'Seller(s)', fields: [
      ...
      ['seller3Name', 'Seller 3 Name'],
      ['seller3Email', 'Seller 3 Email'],
      ['seller3Phone', 'Seller 3 Phone'],
      ['seller4Name', 'Seller 4 Name'],
      ['seller4Email', 'Seller 4 Email'],
      ['seller4Phone', 'Seller 4 Phone'],
    ]},
  ...
  'onboarding': [
    ...
    { title: 'Preferred Vendors', fields: [
      ['prefEscrow', 'Escrow Company'],
      ['prefEscrowName', 'Escrow Name'],
...
export async function sendAgentConfirmation(data, driveResult) {
  if (!data.agentEmail) return;

  // Helper: render a row only if the value is truthy. Keeps the recap table
  // free of empty placeholders when the agent didn't fill in optional fields.
  const row = (label, value) => {
    if (!value) return '';
    return `<tr>
        <td style="padding:8px 12px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;color:#4D0D30;font-size:11px;text-transform:uppercase;width:35%;">${escapeHtml(label)}</td>
        <td style="padding:8px 12px;border:1px solid #E0D5DC;font-size:13px;">${value}</td>
      </tr>`;
  };

  // Format a single client (Name · Email · Phone), only if they have a name.
  const sideKey = (data.side || '').toLowerCase(); // 'buyer' or 'seller'
  const clientLine = (n) => {
    if (!sideKey) return '';
    const name = data[`${sideKey}${n}Name`] || '';
    const email = data[`${sideKey}${n}Email`] || '';
    const phone = data[`${sideKey}${n}Phone`] || '';
    if (!name) return '';
    return [escapeHtml(name), escapeHtml(email), escapeHtml(phone)].filter(Boolean).join(' · ');
  };

  // Document recap: list filenames if any were uploaded; otherwise omit the row.
  const fileList = driveResult?.files || [];
  const docsValue = fileList.length
    ? `${fileList.length} file(s) received:<ul style="margin:4px 0 0 18px;padding:0;">${fileList.map(f => `<li>${escapeHtml(f.name || 'document')}</li>`).join('')}</ul>`
    : '';

  const sideLabel = data.side || 'Client';
  const agentSummary = [data.agentName, data.agentEmail, data.agentPhone].filter(Boolean).map(escapeHtml).join(' · ');
  ...
  <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;">
    ...
    <div style="padding:28px 32px;background:#fff;">
      <p style="font-size:15px;color:#222;">Hi ${escapeHtml(data.agentName?.split(' ')[0] || 'there')},</p>
      <p style="font-size:13px;color:#555;line-height:1.7;">Thanks for sending this over! We've received your file for <strong style="color:#4D0D30;">${escapeHtml(data.propertyAddress || 'your property')}</strong> along with any documents you uploaded. We'll be opening it up shortly — your TC will be in touch very soon with next steps.</p>

      ${data.checklistUrl ? `
      <div style="background:#FAF0F5;border:2px solid #B32D7F;padding:14px 16px;border-radius:4px;margin:18px 0 24px;text-align:center;">
      ...
      <table style="width:100%;border-collapse:collapse;margin:0 0 20px;">
        ${row('File #', data.fileNum ? escapeHtml(data.fileNum) : '')}
        ${row('Property', data.propertyAddress ? escapeHtml(data.propertyAddress) : '')}
        ${row('Transaction Type', data.transactionType ? escapeHtml(data.transactionType) : '')}
        ${row('Submitting Agent', agentSummary)}
        ${row('Brokerage', data.brokerage ? escapeHtml(data.brokerage) : '')}
        ${row(`${sideLabel} 1`, clientLine(1))}
        ${row(`${sideLabel} 2`, clientLine(2))}
        ${row(`${sideLabel} 3`, clientLine(3))}
        ${row(`${sideLabel} 4`, clientLine(4))}
        ${row('Urgent', data.urgent ? '🚨 Yes — flagged for priority' : '')}
        ${row('Notes', data.notes ? escapeHtml(data.notes) : '')}
        ${row('Documents', docsValue)}
      </table>
...
// ─── ALERT: Background checklist setup failed for a real submission ─────────
//
// Fires when appendChecklistRowsToMaster / createPerFileChecklist / etc. fail
// during the waitUntil() background block. The agent already got a 200 + their
// confirmation email, so they're unaffected. But the TC team needs to know
// because the master tab is missing rows + the per-file Checklist Sheet is empty.
//
// Includes a copy-paste recovery curl command (uses /api/admin/replay when
// available; until then, points at the manual replay script).
export async function sendChecklistFailureAlert({ fileNum, side, propertyAddress, agentName, step, errorMessage }) {
  const r = getResend();
  if (!r) return;
  const subject = `⚠️ [${fileNum}] Checklist setup FAILED — needs manual replay`;
  const html = `
  <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;">
    <div style="background:#B32D7F;padding:20px 28px;">
      <h1 style="color:#fff;margin:0;font-size:18px;letter-spacing:1px;">CHECKLIST SETUP FAILED</h1>
      <p style="color:#FAF0F5;margin:4px 0 0;font-size:12px;">A file submission completed but the background checklist step errored after retries.</p>
    </div>
    <div style="padding:24px 28px;background:#fff;border:1px solid #E0D5DC;border-top:none;">
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;width:30%;">File #</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;"><strong>${escapeHtml(fileNum || '?')}</strong></td></tr>
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;">Property</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;">${escapeHtml(propertyAddress || '?')}</td></tr>
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;">Agent</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;">${escapeHtml(agentName || '?')}</td></tr>
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;">Side</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;">${escapeHtml(side || '?')}</td></tr>
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;">Failed step</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;">${escapeHtml(step || 'unknown')}</td></tr>
        <tr><td style="padding:6px 10px;background:#F4F3F2;border:1px solid #E0D5DC;font-weight:700;">Error</td>
            <td style="padding:6px 10px;border:1px solid #E0D5DC;color:#B32D7F;"><code>${escapeHtml((errorMessage || 'no message').slice(0, 300))}</code></td></tr>
      </table>
      <p style="font-size:13px;color:#444;line-height:1.6;margin-top:18px;">
        <strong>What this means:</strong> the agent received their submission confirmation as normal. The Drive folder exists. But the master TC Tasks tab may not have the task rows for this file, and the per-file Checklist Sheet may be empty.
      </p>
      <p style="font-size:13px;color:#444;line-height:1.6;">
        <strong>To recover:</strong> ping Brendan / dev to run the manual replay for this file #. The fix takes ~30 seconds and doesn't disturb anything else.
      </p>
      <p style="font-size:11px;color:#897D76;margin-top:20px;font-style:italic;">
        This alert was auto-generated by api/new-file.js after the background block exhausted retries. If you see this often, the master sheet is hitting API quota limits — let dev know.
      </p>
    </div>
  </div>`;
  try {
    await r.emails.send({ from: FROM, to: TC_EMAIL, subject, html });
  } catch (err) {
    // Don't let the alert-send failure cascade — already in an error path.
    console.error('[sendChecklistFailureAlert] failed to send alert:', err?.message || err);
  }
}
