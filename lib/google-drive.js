...
export async function createPerFileChecklist({
  ...
  // 1. Create the spreadsheet — initially in service-account's My Drive root.
  const createRes = await sheets.spreadsheets.create({
    requestBody: {
      properties: { title: sheetTitle },
      sheets: [{ properties: { title: 'Checklist', gridProperties: { rowCount: 200, columnCount: 11, frozenRowCount: 9 } } }],
    },
    fields: 'spreadsheetId,sheets.properties',
  });
  const newSheetId = createRes.data.spreadsheetId;
  const tabId = createRes.data.sheets[0].properties.sheetId;

  // 2. Move it into the file's Drive folder.
  if (parentFolderId) {
    const meta = await drive.files.get({
      fileId: newSheetId, fields: 'parents', supportsAllDrives: true,
    });
