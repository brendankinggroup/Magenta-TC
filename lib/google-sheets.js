...
export async function appendNewFileRow(data) {
  ...
  const apiRes = await sheets.spreadsheets.values.append({
    ...
    range: '🏠 Active Transactions!A3:CB',
    valueInputOption: 'USER_ENTERED',
    insertDataOption: 'OVERWRITE',
    requestBody: { values: [row] },
  });
  return {
    updatedRange: apiRes?.data?.updates?.updatedRange,
    updatedRows: apiRes?.data?.updates?.updatedRows,
    updatedColumns: apiRes?.data?.updates?.updatedColumns,
    updatedCells: apiRes?.data?.updates?.updatedCells,
    rowLength: row.length,
    rowSampleD_H: row.slice(3, 8),
  };
}
