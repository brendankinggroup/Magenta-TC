...
export async function appendNewFileRow(data) {
  ...
  // Find the next empty row by scanning column C (Status) starting at row 3.
  // We use column C because the code always writes "Open" there, so every
  // real data row has a non-empty C. This avoids the values.append
  // table-detection quirk that writes to unexpected columns.
  ...
  const apiRes = await sheets.spreadsheets.values.update({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
  ...
  return {
    updatedRange: apiRes?.data?.updatedRange,
    nextRow,
