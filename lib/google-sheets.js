...
// Retry a Sheets API call with exponential backoff. Catches transient quota
// collisions and network blips — the failure mode that silently broke 26-023.
// Throws the final error after all attempts fail so callers can alert+recover.
async function withRetry(fn, label) {
  const delays = [2000, 4000, 8000]; // 3 attempts: try, wait 2s, retry, wait 4s, retry, wait 8s, retry
  let lastErr;
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    try {
      ...
      lastErr = err;
      // Don't retry "bad request" type errors — they won't get better
      const status = err?.code || err?.response?.status;
      if (status === 400 || status === 403 || status === 404) {
        console.error(`[${label}] permanent error (${status}), not retrying:`, err?.message);
        throw err;
      }
      if (attempt < delays.length) {
        console.warn(`[${label}] attempt ${attempt + 1} failed (${err?.message}), retrying in ${delays[attempt]}ms`);
        await new Promise(r => setTimeout(r, delays[attempt]));
      }
    }
  }
  console.error(`[${label}] all ${delays.length + 1} attempts failed`);
  throw lastErr;
}
...
export async function appendNewFileRow(data) {
  ...
  // The archive tab is scanned too: closed/cancelled files move there, and
  // without it a fully-archived year would restart numbering at -001 and
  // collide with archived file numbers. Done in parallel to avoid extra
  // round-trips.
  const [probeC, probeA, probeArchive] = await Promise.all([
    ...
    sheets.spreadsheets.values
      .get({
        spreadsheetId: process.env.GOOGLE_SHEET_ID,
        range: `'🗄️ Archived Transactions'!A3:A`,
      })
      .catch(() => null), // tab may not exist in older/staging workbooks
  ]);
  ...
  let maxFileNum = 0;
  const allFileNumRows = [
    ...(probeA.data.values || []),
    ...(probeArchive?.data?.values || []),
  ];
  for (const r of allFileNumRows) {
    const v = (r && r[0]) || '';
...
/**
 * Append a new agent row to the Agent Onboarding sheet (A:AY, 51 cols).
 *
 ...
 * AW: Collects BCF    AX: BCF Amount      AY: Pref Escrow Name (officer name)
 */
export async function appendAgentRow(data) {
  ...
  const row = [
    ...
    safe(data.prefEscrowName),                 // AY (escrow officer name)
  ];
...
export async function getActiveTransactionsChecklistMap() {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: '🏠 Active Transactions!A3:CC',  // data starts at row 3 (row 1 = title banner, row 2 = header row)
  });
...
export async function appendChecklistRowsToMaster({ fileNum, propertyAddress, agentName, side }) {
  ...
  const tplRes = await withRetry(() => sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `'${templateTab}'!A13:F200`,
  }), `appendChecklistRowsToMaster:read-template-${side}`);
  ...
  if (masterRows.length === 0) {
    // A healthy template ALWAYS parses to rows (Buyer 48, Seller 47+2 phase
    // headers). Zero rows means the template tab was empty, mid-edit, or
    // renamed at read time. Throwing (instead of returning count:0) makes the
    // caller's failure alert fire so this can't fail silently — which is
    // exactly how 26-026 lost its checklist on 7/28.
    throw new Error(`appendChecklistRowsToMaster: template '${templateTab}' parsed to 0 task rows — tab empty, mid-edit, or renamed?`);
  }

  // Find next empty row in master tab (probe col A from row 3 onward;
  // rows 1-2 are banner + header).
  const probe = await withRetry(() => sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `'${masterTab}'!A3:A`,
  }), `appendChecklistRowsToMaster:probe-${side}`);
  const existing = probe.data.values || [];
  ...
  }), `appendChecklistRowsToMaster:write-${side}`);
