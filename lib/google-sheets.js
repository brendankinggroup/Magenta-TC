...
export async function appendChecklistRowsToMaster({ fileNum, propertyAddress, agentName, side }) {
  ...
  //   - Phase header: col B starts with "PHASE " (Seller only)
  //   - Task row:     col A is numeric (task #) and col B is non-empty
  //   - Skip:         everything else (blanks, separators)
  let currentPhase = '';
  const masterRows = [];
  for (const r of tplRows) {
    const taskNum = (r[0] || '').toString().trim();
    const task    = (r[1] || '').toString().trim();
    const assigned = (r[5] || '').toString().trim();
    if (/^PHASE\s/i.test(task)) {
      currentPhase = task;
      // Add a phase-header marker row so it's visible in the master view
      masterRows.push([
        safe(fileNum),
        safe(propertyAddress),
        safe(agentName),
        safe(currentPhase),  // D: Phase
        '',                  // E: Task # (blank for header)
        `— ${task} —`,        // F: Task (visual separator)
        '',                  // G: Status
        '',                  // H: Due Date
        '',                  // I: Date Completed
        '',                  // J: Notes
